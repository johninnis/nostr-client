...
To avoid that, each relay's `RelaySession` holds two pieces of state that read like a leak to a
reviewer and invite deletion:
...
stays in `pendingEvents` and its deferred future is moved into the session's `authRetryQueue` rather
than being errored. When the relay later accepts the client's signed auth event, the queue is flushed:
