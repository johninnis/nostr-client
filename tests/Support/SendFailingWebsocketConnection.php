...
/**
 * A websocket whose stream stays open but whose every send fails as the transport
 * would when the socket has broken mid-write, so the send-failure path can be driven.
 *
 * @implements IteratorAggregate<int, WebsocketMessage>
 */
final class SendFailingWebsocketConnection implements WebsocketConnection, IteratorAggregate
{
    ...
    {
        throw new WebsocketClosedException('Client unexpectedly closed', WebsocketCloseCode::ABNORMAL_CLOSE, 'Writing to the client failed');
    }

    #[Override]
    public function receive(?Cancellation $cancellation = null): ?WebsocketMessage
    {
        throw new LogicException('SendFailingWebsocketConnection delivers messages through iteration only');
    }
